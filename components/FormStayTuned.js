...
export default function FormStayTuned({ setModal, modal }) {
  const [inpNameValue, setInpNameValue] = useState("");
  const [inpValue2, setInpValue2] = useState("");
  const [isEmailValid, setIsEmailValid] = useState(false);
  const [isEmpty, setIsEmpty] = useState(true);
  const [isDisabled, setIsDisabled] = useState(true);

  const [danceFest, setDanceFest] = useState(false);
  const [onlineDanceClasses, setOnlineDanceClasses] = useState(false);
  const [videographyServices, setVideographyServices] = useState(false);
  const [artistMentorship, setArtistMentorship] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
      ...
      email: inpValue2,
      name: inpNameValue,
      eventOrganiser: danceFest,
      socialDancer: onlineDanceClasses,
      videographer: videographyServices,
      danceArtist: artistMentorship,
    };
    try {
      const response = await fetch("/api/save-stay-tuned", {
        method: "POST",
        headers: {
      ...
      setInpValue2("");
      setDanceFest(false);
      setOnlineDanceClasses(false);
      setVideographyServices(false);
      setArtistMentorship(false);
    }
  };
    ...
    <div>
      <div className="flex flex-col gap-[24px]">
        <div className="flex justify-between">
          <p className="font-bold text-38px">Stay tuned.</p>
          <div>
            <svg
              className="mr-[10px]"
              width="56"
        ...
        <form onSubmit={handleSubmit} className="mb-[16px] border-[1px] border-[#F2F2F7] border-l-0 border-r-0 border-b-0">
          <p className="text-12px font-regular inline-block mr-2">My name is</p>
          <input
            type="text"
            value={inpNameValue}
            ...
            onChange={(e) => {
              ...
              setInpNameValue(value);
              setIsEmpty(value.trim() === "");
            }}
            className={`mb-[12px] text-12px py-[11px] px-[18px] font-normal rounded-full bg-transparent w-[180px] h-[18px] mt-[10px] border-black border-[1px] border-l-0 border-r-0 border-t-0`}
          />
          <p className="mb-[12px] text-12px font-regular inline-block mr-2"> , and I</p>
          
          <div
            onClick={() => setDanceFest(!danceFest)}
            className={`${
              danceFest
                ? "bg-[#3B3B40] text-[#FFFFFF]"
                : "bg-[#F2F2F7] hover:bg-[#e1e1e7]"
            } text-10px font-regular mr-2 py-[7px] px-[24px] inline-block rounded-full  cursor-pointer text-[#74747C] mb-[12px]`}
          >
            Event Organiser
          </div>
          <div
            onClick={() => setOnlineDanceClasses(!onlineDanceClasses)}
            className={`${
              onlineDanceClasses
                ? "bg-[#3B3B40] text-[#FFFFFF]"
                : "bg-[#F2F2F7] hover:bg-[#e1e1e7]"
          ...
          <div
            onClick={() => setVideographyServices(!videographyServices)}
            className={`${
              videographyServices
                ? "bg-[#3B3B40] text-[#FFFFFF]"
                : "bg-[#F2F2F7] hover:bg-[#e1e1e7]"
          ...
          <div
            onClick={() => setArtistMentorship(!artistMentorship)}
            className={`${
              artistMentorship
                ? "bg-[#3B3B40] text-[#FFFFFF]"
                : "bg-[#F2F2F7] hover:bg-[#e1e1e7]"
          ...
          <br />
          <div className="border-[#F2F2F7] border-[1px] border-l-0 border-r-0 border-t-0 mb-[12px]">
          <p className="text-12px font-regular inline-block mr-2">You can reach me at</p>
          ...
          <input
            ...
            className={` inline-block text-12px py-[11px] px-[18px] font-normal bg-transparent w-[180px] mb-[12px] h-[18px] mt-[10px] border-black border-[1px] rounded-full border-l-0 border-r-0 border-t-0`}
          />
          </div>
          <button
            ...
            type="submit"
            className={cn(
              "text-14px font-semibold bg-customBlue rounded-full py-[11px] w-full text-white tracking-widest transition duration-500 ease-in-out",
